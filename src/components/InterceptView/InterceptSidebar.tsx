  ...
  if (collapsed) {
    return (
      <aside data-testid="sidebar" className="bg-app-sidebar text-app-inverse overflow-hidden p-2 flex flex-col items-center gap-2 h-full">
        <button type="button" onClick={onToggle} className="p-2 hover:bg-app-subtle rounded transition-colors cursor-pointer" aria-label="Expand sidebar" title="Expand sidebar (Cmd/Ctrl+B)"><ChevronRight size={20} className="text-app-muted" /></button>
        ...
        <button type="button" onClick={() => { setActivePanel("breakpoints"); onToggle() }} className="p-2 hover:bg-app-subtle rounded transition-colors cursor-pointer" aria-label="Breakpoints" title={t('breakpoints.title')}><Bug size={20} className="text-app-muted" /></button>
        <button type="button" onClick={() => { setActivePanel("mappings"); onToggle() }} className="p-2 hover:bg-app-subtle rounded transition-colors cursor-pointer" aria-label="Mappings" title={t('mappings.title')}><Route size={20} className="text-app-muted" /></button>
        <button type="button" onClick={() => { setSidebarSettingsExpanded(true); void saveSetting("sidebar_settings_expanded", true); onToggle() }} className="p-2 hover:bg-app-subtle rounded transition-colors cursor-pointer mt-auto" aria-label="Settings" title={t('settings.title')} data-testid="collapsed-settings-button"><SettingsIcon size={20} className="text-app-muted" /></button>
      </aside>
    );
  }
  ...
  return (
    <aside data-testid="sidebar" className="bg-app-sidebar text-app-inverse overflow-hidden p-3 flex flex-col h-full">
      <div className="flex items-center justify-between mb-3">
        <button type="button" onClick={onToggle} className="p-1.5 hover:bg-app-subtle rounded transition-colors cursor-pointer" aria-label="Collapse sidebar" title="Collapse sidebar (Cmd/Ctrl+B)"><ChevronLeft size={18} className="text-app-muted" /></button>
      ...
      <div className="flex shrink-0 mb-3 rounded overflow-hidden border border-app-subtle">
        <button type="button" onClick={() => setActivePanel("breakpoints")} data-tour="breakpoints-panel" className={`flex-1 py-1.5 text-xs cursor-pointer ${activePanel === "breakpoints" ? "bg-app-subtle text-app-inverse font-medium" : "text-app-muted hover:text-app-inverse"}`} aria-label="Breakpoints panel">{t('breakpoints.title')}</button>
        <button type="button" onClick={() => setActivePanel("mappings")} data-tour="mappings-panel" className={`flex-1 py-1.5 text-xs cursor-pointer ${activePanel === "mappings" ? "bg-app-subtle text-app-inverse font-medium" : "text-app-muted hover:text-app-inverse"}`} aria-label="Mappings panel">{t('mappings.title')}</button>
      </div>
