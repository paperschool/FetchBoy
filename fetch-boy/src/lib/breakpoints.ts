...
export async function createBreakpoint(
    ...
    const ts = now();
    await db.execute(
        'INSERT INTO breakpoints (id, folder_id, name, url_pattern, match_type, enabled, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [id, folderId, name, urlPattern, matchType, 1, ts, ts],
    );
    return { id, folder_id: folderId, name, url_pattern: urlPattern, match_type: matchType, enabled: true, created_at: ts, updated_at: ts };
}

export async function updateBreakpoint(
    id: string,
    changes: Partial<Pick<Breakpoint, 'name' | 'url_pattern' | 'match_type' | 'enabled'>>,
): Promise<void> {
    const db = await getDb();
    const parts: string[] = [];
