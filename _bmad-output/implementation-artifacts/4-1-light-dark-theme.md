...
- [ ] Task 1 — Create `lib/settings.ts` (AC: 1)
  - [ ] Create `dispatch/src/lib/settings.ts`
  - [ ] Export `loadAllSettings(): Promise<AppSettings>` — reads all 4 keys from SQLite `settings` table, parsing each JSON-encoded value; returns default `AppSettings` object on failure
  - [ ] Export `saveSetting(key: keyof AppSettings, value: AppSettings[keyof AppSettings]): Promise<void>` — serialises to JSON string (`JSON.stringify(value)`) and upserts via `INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)`
  - [ ] Use `getDb()` from `@/lib/db` — **do not** create a new `Database.load()` call
  - [ ] Default values if DB read fails: `{ theme: 'system', request_timeout_ms: 30000, ssl_verify: true, editor_font_size: 14 }`

- [ ] Task 2 — Extend `uiSettingsStore.ts` with theme state (AC: 1, 3)
  - [ ] Open `dispatch/src/stores/uiSettingsStore.ts`
  - [ ] Add `theme: 'light' | 'dark' | 'system'` to `UiSettingsState` interface
  - [ ] Add `setTheme: (theme: 'light' | 'dark' | 'system') => void` to the interface
  - [ ] Initialise `theme` with `'system'` as default (overridden at app startup from DB)
  - [ ] Implement `setTheme` with `set({ theme })` — **do not** call `saveSetting` from inside the store action; persistence is triggered at the call site (in the toggle handler)
  - [ ] **Do not** use Immer for this store — it currently uses plain Zustand `set`; keep pattern consistent

- [ ] Task 3 — Create `useTheme` hook (AC: 2, 3, 4)
  - [ ] Create `dispatch/src/hooks/useTheme.ts`
  - [ ] Import `useUiSettingsStore` and `useEffect`
  - [ ] Inside a `useEffect([theme])`, apply the `.dark` class to `document.documentElement`:
  ...
  - [ ] Export the hook as `useTheme`
  - [ ] The hook returns nothing — it is side-effect only

- [ ] Task 4 — Bootstrap settings at app startup in `AppShell.tsx` (AC: 1, 2)
  - [ ] Open `dispatch/src/components/Layout/AppShell.tsx`
  - [ ] Import `loadAllSettings` from `@/lib/settings` and `useUiSettingsStore`
  - [ ] In the existing `useEffect([], [])`, after `loadAllEnvironments`, load settings:
    ```ts
    loadAllSettings()
      .then((s) => {
    ...
    ```
  - [ ] Call `useTheme()` at the top of `AppShell` component body (hook subscribes to `theme` changes and mutates `document.documentElement`)
  - [ ] **Do not** remove the existing environment-loading `useEffect` logic

- [ ] Task 5 — Wire Monaco Editor theme (AC: 5)
  - [ ] Open `dispatch/src/components/Editor/MonacoEditorField.tsx`
  - [ ] Read `theme` from `useUiSettingsStore`
  - [ ] Derive `monacoTheme`:
    - `theme === 'dark'` → `"vs-dark"`
    - `theme === 'system'` → `window.matchMedia('(prefers-color-scheme: dark)').matches ? "vs-dark" : "vs"`
    - `theme === 'light'` → `"vs"`
  - [ ] Pass `monacoTheme` as the `theme` prop to the `<Editor>` component from `@monaco-editor/react`
  - [ ] The Monaco theme must **re-evaluate** when `theme` store value changes — React's reactivity via `useUiSettingsStore` subscriber handles this automatically

- [ ] Task 6 — Add theme toggle to TopBar (AC: 6)
  - [ ] Open `dispatch/src/components/TopBar/TopBar.tsx`
  - [ ] Import `useUiSettingsStore` and `saveSetting` from `@/lib/settings`
  - [ ] Read `theme` from store: `const theme = useUiSettingsStore((s) => s.theme)`
  - [ ] Add `handleThemeChange` async function that cycles `none → light → dark → system → light`:
    ```ts
  ...
  - [ ] Add a small icon button to the right of the gear icon (⚙) in the TopBar JSX:
    ...
    ```
  - [ ] Place this button **before** the existing `⚙` environment button so the order is: theme toggle | env selector | env settings

- [ ] Task 7 — Tests (AC: 1–6)
  - [ ] Create `dispatch/src/lib/settings.test.ts`:
  ...
  - [ ] Create `dispatch/src/hooks/useTheme.test.ts`:
  ...
  - [ ] Extend `dispatch/src/stores/uiSettingsStore.test.ts` (or create if not present):
    - Test: `setTheme` updates `theme` in store
    - Test: initial `theme` value is `'system'`
  - [ ] Extend `dispatch/src/components/TopBar/TopBar.test.tsx`:
...
- [ ] Final Task — Commit story changes
  - [ ] Commit all code and documentation changes for this story with a message that includes Story 4.1
