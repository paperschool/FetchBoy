...
- **Always run `npx tsc -b` before considering a change complete** — this matches the CI build command and enforces `noUnusedLocals`/`noUnusedParameters`. Do NOT use `tsc --noEmit` as it skips these checks. Build failures are unacceptable — catch them locally, not in CI.
